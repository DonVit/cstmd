...
class StatsWebPage extends MainWebPage {
	...
	function getSessionsByDate(){
		...
		$out.='<tr><th>Session</th><th>Session Start</th><th>ip</th><th>Queries</th></tr>';
		//$out.='<tr><th class="gridth">id</th><th>Session Start</th><th>Session</th><th>ip</th><th>Queries</th><th>Duration</th></tr>';
		if (count($vs)!=0){
			$i=count($vs);
			foreach($vs as $v){
				
				$out.='<tr class="gridtr'.($i & 1).'"><td class="gridtd"><a href="'.Config::$statssite.'/queries.php?session='.$v->session.'">'.$i.'</td><td>'.$v->datetime.'</td><td class="gridtd">'.$v->ip.'</td><td class="gridtd">'.$v->queries.'</td></tr>';
				$i--;
