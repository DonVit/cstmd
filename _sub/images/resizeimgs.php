		...
		if ($hndDir = opendir($srcdir)){
			...
			while (false !== ($strFilename = readdir($hndDir))){
				if ($strFilename != "." && $strFilename != ".."){
					...
					Image::makeIcons_MergeCenter($srcdir."/".$strFilename, $dstdir."/".$strFilename, 600);
		...
		if ($hndDir = opendir($srcdir)){
			...
			while (false !== ($strFilename = readdir($hndDir))){
				if ($strFilename != "." && $strFilename != ".."){
					...
					//Image::makeIcons_MergeCenter($srcdir."/".$strFilename, $dstdir."/".$strFilename, 600);
					//Image::makeIcons_MergeCenter($srcdir."/".$strFilename, $dstdir."/s".substr($strFilename,1), 300);
					$i=filesize($srcdir."/".$strFilename);
					$o=filesize($dstdir."/".$strFilename);
					if ($o>$i){
						unlink($dstdir."/".$strFilename);
						$out.="<tr><td>".$strFilename."</td><td>".filesize($srcdir."/".$strFilename)."</td><td>".filesize($dstdir."/".$strFilename)."</td></tr>";
					}
					$intCount++;
				}
			}
