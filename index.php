...
class IndexWebPage extends MainWebPage {
	...
	function getLastNews(){
		...
		foreach($ns as $n){
		   		...
		   		//$out.='<h2 class="groupheader_h2"><a href="'.Config::$newssite.'/index.php?id='.$n->news_id.'" class="title" target="_self">'.$n->title.'</a></h2>';
		   		$out.='<h2 class="groupheader_h2"><a href="'.$this->getUrl(Config::$newssite.'/index.php','action=viewnews&id='.$n->news_id).'" class="title" target="_self">'.$n->title.'</a></h2>';
		  		...
		  		if ($i!=null){
		  			...
		  			//$out.='<table align="left" style="margin-right:6px;"><tr><td style="vertical-align: top;border: 1px;border-color: red;border-style: solid;margin-right:5px;"><img src="'.Config::$newssite.'/images/'.$n->image_file.'" alt="'.$n->image_description.'" style="hight:100px; width:150px;"></img></td></tr></table>';
		  		...
		  		$out.='<span id="spantext">'.$n->t.'</span> ... <a href="'.$this->getUrl(Config::$newssite.'/index.php','action=viewnews&id='.$n->news_id).'" target="_self">mai mult</a></td></tr>';
		...
		$out.='</td></tr></table>';
		$out.='<h2 class="groupfooter_h2"><a href="'.$this->getUrl(Config::$newssite.'/index.php').'">mai multe stiri</a></h2>';		
		$out.='</div>';
	...
	function getLastImobil(){
		...
		foreach($rs as $n){
		   		...
		   		//$out.='<h2 class="groupheader_h2"><a href="'.Config::$newssite.'/index.php?id='.$n->id.'" class="title" target="_self">'.$n->scop_name.', Se vinde, '.$n->tipimobil_name.'-'.$n->subtipimobil_name.', '.($n->municipiu==1?echo 'm.':echo 'r.').$n->raion_name.', '.$n->localitate_name.'</a></h2>';
		...
		$out.='</td></tr></table>';
		$out.='<h2 class="groupfooter_h2"><a href="'.$this->getUrl(Config::$imobilsite.'/index.php').'">mai multe imobile</a></h2>';
		$out.='</div>';	
		return $out;			
	}
	function getLastChirie(){
		...
		$out.='</td></tr></table>';
		$out.='<h2 class="groupfooter_h2"><a href="'.$this->getUrl(Config::$chiriesite.'/index.php').'">mai multe chirii</a></h2>';
		$out.='</div>';	
		return $out;			
	}
	...
	function getLastImages(){
		...
		foreach($ps as $p){
			...
			if ($i==4){
	  		...
	  		$out.='<div><a href="'.$this->getUrl(Config::$imagessite.'/index.php','action=viewimage&id='.$p->id).'"><img src="'.Config::$imagessite.'/files/t'.$p->file.'" alt="'.$p->title.'" style1="border: 2px solid #C3D9FF;padding:5px;" class="imageborder"></img><p style="font-size:80%;">'.$p->title.'</p></a></div>';	  		
	  		$out.='</td>';
			if ($i==8){
				$out.="</tr>";
			}
		...
		$out.="</tr></table>";
		$out.='<h2 class="groupfooter_h2"><a href="'.$this->getUrl(Config::$imagessite.'/index.php').'">mai multe imagini</a></h2>';		
		$out.="</div>";
		return $out;
	}
	...
	function getImobilAdd(){
		...
		$out.='<ul class="leftmenulist">';		
		$out.='<li><a href="'.$this->getUrl(Config::$imobilsite.'/add.php').'">Adauga Oferta Imobil</a></li>';
		//$out.='<li><a href="'.Config::$companiesite.'">Adauga Oferta Chirie</a></li>';		
		$out.='<li><a href="'.$this->getUrl(Config::$imobilsite.'/add.php','scop_id=3').'">Adauga Cerere Imobil</a></li>';
	...
	function getChirieAdd(){
		...
		$out.='<h2 class="groupheader_h2">Adauga Anunt:</h2>';												
		$out.='<ul class="leftmenulist">';		
		$out.='<li><a href="'.$this->getUrl(Config::$chiriesite.'/add.php','scop_id=2').'">Adauga Oferta Chirie</a></li>';
		//$out.='<li><a href="'.Config::$companiesite.'">Adauga Oferta Chirie</a></li>';		
		$out.='<li><a href="'.$this->getUrl(Config::$chiriesite.'/add.php','scop_id=4').'">Adauga Cerere Chirie</a></li>';
		//$out.='<li><a href="'.Config::$companiesite.'">Adauga Cerere Chirie</a></li>';				
		$out.='</ul>';		
		$out.='<h2 class="groupfooter_h2">...</h2>';
		$out.='</div>';
