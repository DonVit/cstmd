...
function OnMapLoad() {
	...
	if (GBrowserIsCompatible()) {
    	...
    	var l=document.getElementById("localitate_id").value;
 ...
 //map.setCenter(new GLatLng(centerlat, centerlng), zoom,  map.getMapTypes()[maptype]);   			
		...
		GDownloadUrl('locationsmapingxml.php?localitateid='+l, function(data) {
			var l=document.getElementById("localitate_id").value;
			...
			for (var i = 0; i < markers.length; i++) {
				...
				var centerlat = markers[i].getAttribute("centerlat");
				var centerlng = markers[i].getAttribute("centerlng");
				var point = new GLatLng(parseFloat(markers[i].getAttribute("lat")), parseFloat(markers[i].getAttribute("lng")));
				var marker = createMarker(point, title, l, id);
...
function createMarker(point, name, lid, nid) {
  var marker = new GMarker(point);
  //var marker = new GMarker(point, customIcons[type]);  
  //var html = "<b>" + name + "</b> <br/>" + address+ "<br/><a href=\""+link+"\">"+link+"</a>";
  var html = "<b>" + name + "</b><br><form id=\"poiform\" name=\"poiform\" method=\"post\"><input name=\"lid\" value=\""+lid+"\"/><br><input name=\"nid\" value=\""+nid+"\"/><br><input type=\"submit\" name=\"save\" value=\"Save\"/></form>";  
